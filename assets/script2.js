...
teamCrowdCheck.addEventListener('click', function (e) {
  ...
  newButton.addEventListener('click', function (e) {
    ...
    fetch(api).then(triMonstre)

    function triMonstre (responseText) {
      if (responseText.ok) {
        responseText
          .json()
...
//on filtre les monstres parmis lesquels faire une selection aléatoire en plaçant les objets dans un tableau         
    ...
    const filterMonstersIndex = data => {
      for (const monster of data.results) {
        ...
        fetch(monsterInfos).then(travailFetch).then(selectRndMob)

        
    }
}
/*================= modif pierre :
 extraction des deux fonstions ci dessous du code de la fonction 
 teamCrowdCheck.addEventListener('click', function (e){...})
 */
